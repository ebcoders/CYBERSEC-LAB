...
void decryptFile(const std::string& inputFile, const std::string& outputFile, 
    ...
    try {
        ...
        decryptor.SetKeyWithIV(keyBytes, keyBytes.size(), ivBytes);
...
int main() {
    ...
    std::cout << "Enter input encrypted file name: ";
    std::cin >> inputFile;
    std::cout << "Enter output JPEG file name: ";
    std::cin >> outputFile;
    ...
    try {
        ...
        decryptFile(inputFile, outputFile, decodedKey, decodedIV);
