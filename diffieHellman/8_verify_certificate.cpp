...
bool IsDateWithinRange(const std::string& notBefore, const std::string& notAfter) {
    ...
    std::istringstream ssNotBefore(notBefore);
    ssNotBefore >> std::get_time(&tm, "%a, %d %b %Y");
    std::time_t notBeforeTime = std::mktime(&tm);

    std::istringstream ssNotAfter(notAfter);
    ssNotAfter >> std::get_time(&tm, "%a, %d %b %Y");
    std::time_t notAfterTime = std::mktime(&tm);
